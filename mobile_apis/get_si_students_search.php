<?php
/**
 * Student Information — student list (active students), mirrors web "Student Details" search.
 * GET: mode = filter | full
 *   filter: class_id (required), section_id (optional, 0 = all sections in class for session)
 *   full: search_text (required, min 2 chars) — matches names, admission_no, roll, phones, email
 */
...
try {
    ...
    if ($mode === 'filter') {
        ...
        $sql .= " ORDER BY st.admission_no ASC";
        ...
        while ($row = $result->fetch_assoc()) {
            $students[] = si_map_student_row($row);
        }
    } elseif ($mode === 'full') {
        ...
        $sql = "SELECT DISTINCT
            ...
            WHERE ss.session_id = " . $session_id . "
              ...
              AND (
                ...
                OR st.father_name LIKE '" . $like . "'
                OR st.guardian_name LIKE '" . $like . "'
                OR st.guardian_phone LIKE '" . $like . "'
...
function si_map_student_row($row) {
    return [
