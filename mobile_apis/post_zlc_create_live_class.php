...
try {
    ...
    $staff_id_host = isset($body['staff_id']) ? (int) $body['staff_id'] : 0;
    $created_id = isset($body['created_id']) ? (int) $body['created_id'] : 0;
    $section_ids = isset($body['section_ids']) && is_array($body['section_ids']) ? $body['section_ids'] : array();
    if ($title === '' || $date_in === '' || $duration <= 0 || $staff_id_host <= 0 || $created_id <= 0 || count($section_ids) === 0) {
        throw new Exception('title, date, duration, staff_id, created_id, section_ids required');
    }
    ...
    $sql = "INSERT INTO conferences (staff_id, title, `date`, duration, password, created_id, api_type, host_video, client_video, description, timezone, return_response, session_id, purpose, status)
        VALUES (" . (int) $staff_id_host . ", '" . $titleEsc . "', '" . $date_sql . "', " . (int) $duration . ", '" . $pwdEsc . "', " . (int) $created_id . ", '" . $mysqli->real_escape_string($api_type) . "', " . (int) $host_video . ", " . (int) $client_video . ", '" . $descEsc . "', '" . $tzEsc . "', '" . $ret . "', " . (int) $session_id . ", 'class', 0)";
    if (!$mysqli->query($sql)) {
    ...
    foreach ($section_ids as $sid) {
        ...
        if (!$mysqli->query('INSERT INTO conference_sections (conference_id, cls_section_id) VALUES (' . $newId . ', ' . $sid . ')')) {
            throw new Exception('Insert section failed: ' . $mysqli->error);
        }
    }
    $mysqli->commit();
