<?php
/**
 * Upload Feedback File API
 * Accepts multipart/form-data with key "file". Use for voice recordings or document attachments.
 * Returns public URL of saved file.
 * Allowed: audio (m4a, mp3, wav, aac), documents (pdf, doc, docx, etc.)
 */
...
if ($ext === '' || !in_array($ext, $allowed_exts)) {
    echo json_encode(['success' => false, 'error' => 'Invalid file type. Allowed: documents (PDF, DOC, etc.) and audio (M4A, MP3, WAV, etc.)']);
    exit;
}
if ($file['size'] > $max_size) {
    echo json_encode(['success' => false, 'error' => 'File too large. Max 15 MB']);
    exit;
}

$safe_name = preg_replace('/[^a-zA-Z0-9._-]/', '_', basename($name, '.' . $ext));
$save_name = uniqid('fb_', true) . '_' . substr($safe_name, 0, 32) . '.' . $ext;
...
$url_path = '/uploads/feedback/' . $save_name;
