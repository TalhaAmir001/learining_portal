    ...
    function getChatUserId($user_id, $user_type = 'staff')
    {
        ...
        $col = (in_array($user_type, ['staff', 'admin'], true)) ? 'staff_id' : (($user_type === 'teacher') ? 'teacher_id' : (in_array($user_type, ['guardian', 'parent'], true) ? 'parent_id' : 'student_id'));
    ...
    try {
        ...
        if ($chat_user_one === null || $chat_user_two === null) {
            ...
            // Validate user types (teacher/guardian talk to admins via Support only)
            $user_one_type = strtolower($user_one_type);
            $user_two_type = strtolower($user_two_type);
            $valid_types = ['student', 'guardian', 'teacher', 'admin', 'staff', 'parent'];
