...
class Notification_model extends MY_Model
{
    ...
    public function getVisibleForStudents()
    {
        $this->db->select('send_notification.*');
        $this->db->from('send_notification');
        $this->db->where_in('visible_student', array('yes', 'Yes'));
        $this->db->order_by('send_notification.id', 'desc');
        $query = $this->db->get();
        return $query->result_array();
    }

    /**
     * Get a single notification by id (no role filter). For API use.
     * @param int $id
     * @return array|null
     */
    public function getById($id)
    {
        $query = $this->db->where('id', (int) $id)->get('send_notification');
    ...
    {
        $studentid  = $this->customlib->getStudentSessionUserID();
        $session_id = $this->setting_model->getCurrentSession();
        $studentid  = $this->db->escape($studentid);
        $session_id = $this->db->escape($session_id);
        $sql = "SELECT send_notification.*
            FROM send_notification
            LEFT JOIN read_notification ON read_notification.student_id = " . $studentid . " AND read_notification.notification_id = send_notification.id
            LEFT JOIN student_session ON student_session.student_id = " . $studentid . " AND student_session.session_id = " . $session_id . "
            WHERE send_notification.visible_student IN ('yes', 'Yes')
            AND read_notification.id IS NULL
            AND (
                (send_notification.class_id IS NULL)
                OR (send_notification.class_id IS NOT NULL
                    AND student_session.class_id = send_notification.class_id
                    AND (
                        (send_notification.section_ids IS NOT NULL AND TRIM(IFNULL(send_notification.section_ids,'')) != ''
                            AND FIND_IN_SET(student_session.section_id, send_notification.section_ids))
                        OR ((send_notification.section_ids IS NULL OR TRIM(IFNULL(send_notification.section_ids,'')) = '')
                            AND send_notification.section_id IS NOT NULL
                            AND student_session.section_id = send_notification.section_id)
                    ))
            )
            GROUP BY send_notification.id
...
LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id AND read_notification.student_id = " . $studentid . "
LEFT JOIN student_session ON student_session.student_id = " . $studentid . " AND student_session.session_id = " . $session_id . "
WHERE send_notification.visible_student IN ('Yes', 'yes')
AND (
    (send_notification.class_id IS NULL)
...
LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id and read_notification.parent_id=" . $this->db->escape($parentid) . " where send_notification.visible_parent='Yes' order by send_notification.publish_date desc");
    ...
    public function countUnreadNotificationStudent($studentid = null)
    {
        ...
        $query = $this->db->query("SELECT * FROM (
            SELECT IF (read_notification.id IS NULL,'unread','read') as notification_id
            FROM send_notification
            LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id AND read_notification.student_id = " . $studentid . "
            LEFT JOIN student_session ON student_session.student_id = " . $studentid . " AND student_session.session_id = " . $session_id . "
            WHERE send_notification.visible_student IN ('Yes', 'yes')
            AND send_notification.publish_date <= " . $date . "
            AND (
                (send_notification.class_id IS NULL)
                OR (send_notification.class_id IS NOT NULL
                    AND student_session.class_id = send_notification.class_id
        ...
        ) final WHERE notification_id = 'unread'");
    ...
    public function insertBatch($data, $staff_roles, $delete_array = array())
    {
        ...
        // Return send_notification.id so callers (e.g. Notification::_sendNoticeFcm) pass the correct id to FCM/WebSocket.
        // Returning true made (int)true === 1 and targeted pushes used notification_roles for id 1 only.
        return $insert_id;
    }
